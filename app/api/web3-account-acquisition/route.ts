...
export async function GET(request: Request): Promise<NextResponse<Web3AccountAcquisitionReceipt | { error: string }>> {
  ...
  return NextResponse.json(buildWeb3AccountAcquisitionReceipt(state));
}
