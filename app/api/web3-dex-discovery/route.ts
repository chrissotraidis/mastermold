...
import { buildWeb3DexDiscoveryReceipt, type Web3DexDiscoveryReceipt } from "@/src/db/web3-dex-discovery";
import {
...
export async function GET(request: Request): Promise<NextResponse<Web3DexDiscoveryReceipt | { error: string }>> {
  ...
  return NextResponse.json(buildWeb3DexDiscoveryReceipt(state));
}
