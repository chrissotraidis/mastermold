...
import {
  ...
  type Web3LiveUsabilityBlockersReceipt,
...
export async function GET(request: Request): Promise<NextResponse<Web3LiveUsabilityBlockersReceipt | { error: string }>> {
  ...
  const liveOps = buildWeb3LiveOpsPacket({
    ...
    emergencyStop: buildWeb3EmergencyStopDrillReceipt({
      reason: "live usability blocker preview",
      operator_ack: true,
    }),
    accounting,
  });
