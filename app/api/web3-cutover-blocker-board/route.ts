...
import {
  buildWeb3CutoverBlockerBoard,
  type Web3CutoverBlockerBoard,
} from "@/src/db/web3-cutover-blocker-board";
...
export async function GET(request: Request): Promise<NextResponse<Web3CutoverBlockerBoard | { error: string }>> {
  ...
  const liveOps = buildWeb3LiveOpsPacket({
    ...
    emergencyStop: buildWeb3EmergencyStopDrillReceipt({
      reason: "cutover blocker board preview",
      operator_ack: true,
    }),
    accounting,
  });
  ...
  return NextResponse.json(buildWeb3CutoverBlockerBoard({
    requestPacket,
    runway,
    usability,
  }));
