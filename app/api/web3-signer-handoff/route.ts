...
import { buildWeb3SignerHandoffReceipt, type Web3SignerHandoffReceipt } from "@/src/db/web3-signer-handoff";
import {
...
export async function GET(request: Request): Promise<NextResponse<Web3SignerHandoffReceipt | { error: string }>> {
  ...
  return NextResponse.json(buildWeb3SignerHandoffReceipt(state));
}
