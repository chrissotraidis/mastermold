...
import { buildWeb3ProviderHealthReceipt, type Web3ProviderHealthReceipt } from "@/src/db/web3-provider-health";
import {
...
export async function GET(request: Request): Promise<NextResponse<Web3ProviderHealthReceipt | { error: string }>> {
  ...
  return NextResponse.json(await buildWeb3ProviderHealthReceipt(state));
}
