...
const docsByService: Record<IntegrationService, string> = {
  ...
  robinhood: "https://docs.snaptrade.com/reference/Connections/Connections_listBrokerageAuthorizations",
  onchain_wallet: "https://developers.zerion.io/reference/authentication",
...
async function testCoinbase(body: Record<string, unknown> | null): Promise<TestResult> {
  ...
  return ok("coinbase", "Coinbase read-only account test passed.", `${json.accounts?.length ?? 0} accounts returned.`);
}
...
async function testSnapTrade(body: Record<string, unknown> | null): Promise<TestResult> {
  ...
  return ok("robinhood", "SnapTrade connection list test passed.", summarizeSnapTradeConnections(json));
}
...
function summarizeSnapTradeConnections(value: unknown) {
  if (!Array.isArray(value)) return "Connection response returned. Use the import button to pull a holdings snapshot.";
  ...
  return `${value.length} connection${value.length === 1 ? "" : "s"} returned: ${readOnly} read-only, ${tradeEnabled} trade-capable${unknown > 0 ? `, ${unknown} unknown` : ""}. Master Mold ignores trading capability and never calls order endpoints. Use the import button to pull a holdings snapshot.`;
}
