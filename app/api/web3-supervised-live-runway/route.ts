...
export async function GET(request: Request): Promise<NextResponse<Web3SupervisedLiveRunway | { error: string }>> {
  ...
  const liveOps = buildWeb3LiveOpsPacket({
    ...
    emergencyStop: buildWeb3EmergencyStopDrillReceipt({
      reason: "supervised live runway preview",
      operator_ack: true,
    }),
    accounting,
  });

  return NextResponse.json(buildWeb3SupervisedLiveRunway({
    state,
