...
import { buildWeb3AccountingLedgerReceipt, type Web3AccountingLedgerReceipt } from "@/src/db/web3-accounting-ledger";
import {
...
export async function GET(request: Request): Promise<NextResponse<Web3AccountingLedgerReceipt | { error: string }>> {
  ...
  return NextResponse.json(buildWeb3AccountingLedgerReceipt(state));
}
