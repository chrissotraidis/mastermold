...
const todayContext: ChatPageContext = {
  surface: "Today",
  route: "/",
  summary: "Today briefing and alpha chat.",
};
...
describe("chat route hints", () => {
  ...
  test("GIVEN natural user wording WHEN typed THEN Master Mold shows matching app routes before submit", () => {
    ...
    expect(routeHintsForChatDraft("check trade").map((hint) => hint.href)).toContain("/trading");
    ...
    expect(routeHintsForChatDraft("run kill switch drill").map((hint) => hint.href)).toContain("/trading?action=run-kill-switch-drill");
    expect(routeHintsForChatDraft("trade safety drill").map((hint) => hint.href)).toContain("/trading?action=run-kill-switch-drill");
    expect(routeHintsForChatDraft("run paper test").map((hint) => hint.href)).toContain("/paper#paper-trade-form");
    ...
    expect(routeHintsForChatDraft("Next action")[0]).toEqual({ label: "Open Trade", href: "/trading" });
    expect(routeHintsForChatDraft("Wallet status")[0]).toEqual({ label: "Open Trade", href: "/trading" });
    expect(routeHintsForChatDraft("Set up wallet")[0]).toEqual({ label: "Open Trade", href: "/trading" });
    ...
    expect(routeHintsForChatDraft("show technical status", tradeContext).map((hint) => hint.href)).toContain("/trading");
    ...
    expect(submittableCommandRoutesForChatDraft("Run safety drill.")[0]).toEqual({
      ...
      href: "/trading?action=run-kill-switch-drill",
    });
  ...
  test("GIVEN an explicit navigation command WHEN submitted THEN Master Mold can route immediately", () => {
    ...
    expect(directRouteForChatDraft("What should I focus on today?", todayContext)).toBeNull();
    expect(commandRoutesForChatDraft("What should I focus on today?", todayContext)).toEqual([]);
    expect(submittableCommandRoutesForChatDraft("What should I focus on today?", todayContext)).toEqual([]);
  });
});
