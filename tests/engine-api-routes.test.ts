...
import { GET as getPortfolioRoute, POST as postPortfolioRoute } from "@/app/api/portfolio/route";
...
describe("API routes serve engine output end to end", () => {
  ...
  test("GET /api/portfolio stays seeded — the engine never touches holdings", async () => {
    ...
    expect(portfolio.daily_review.source_label).toBe("Sample fallback");
    expect(portfolio.daily_review.portfolio_brain_status).toBe("sample");
    expect(portfolio.daily_review.freshness_label).toContain("Sample fallback");
    expect(portfolio.daily_review.read_only).toBe(true);
    expect(portfolio.daily_review.data_boundary).toContain("cannot place brokerage trades");
    expect(portfolio.daily_review.recommendations.length).toBeGreaterThan(0);
    expect(portfolio.daily_review.recommendations.every((item) => item.data_boundary.includes("cannot place brokerage trades"))).toBe(true);
  ...
  test("POST /api/portfolio reports manual holdings as the active daily review source", async () => {
    const portfolio = await json<{
      provenance: { label: string };
      holdings: Array<{ symbol: string; source_label: string }>;
      manual_entries: Array<{ symbol: string; source_label: string }>;
      daily_review: {
        source_label: string;
        portfolio_brain_status: string;
        freshness_label: string;
    ...
    expect(portfolio.provenance.label).toBe("Manual portfolio");
    expect(portfolio.manual_entries).toContainEqual(expect.objectContaining({ symbol: "AAPL", source_label: "Manual entry" }));
    expect(portfolio.holdings).toContainEqual(expect.objectContaining({ symbol: "AAPL", source_label: "Manual entry" }));
    expect(portfolio.daily_review.source_label).toBe("Manual holdings");
    expect(portfolio.daily_review.portfolio_brain_status).toBe("manual");
    expect(portfolio.daily_review.freshness_label).toContain("Manual holdings entered locally");
    expect(portfolio.daily_review.as_of).toMatch(/2026-/);
    expect(portfolio.daily_review.data_boundary).toContain("Manual rows are not broker-connected");
    expect(portfolio.daily_review.data_boundary).toContain("cannot place brokerage trades");
  });
  ...
  test("POST /api/chat keeps market-scan commands quick and safe when the runner is unavailable", async () => {
    ...
    try {
      ...
      expect(text).toContain("read-only portfolio preflight");
      expect(text).toContain("syncs Monarch if available");
  ...
  test("POST /api/chat does not let Trade page context steal explicit non-Trade commands", async () => {
    const cases = [
      ...
      },
      {
        ...
        expectedAction: { label: "Open Trade", href: "/trading" },
      },
    ];
  ...
  test("POST /api/chat routes direct app actions to their primary flows", async () => {
    const cases = [
      ...
      },
      {
        ...
        expectedAction: { label: "Autopilot Settings", href: "/settings#web3-wallet-trading" },
      },
      {
        message: "open web3 setup",
        expectedText: "Autopilot setup:",
        expectedAction: { label: "Autopilot Settings", href: "/settings#web3-wallet-trading" },
      },
      {
    ...
    try {
      const res = await postChatRoute(
        new Request("http://localhost/api/chat", {
          method: "POST",
          body: JSON.stringify({
            // Command-shaped on purpose: with a live key configured, natural
            // questions now stream to the model; only commands stay canned.
            messages: [{ role: "user", content: "Today check" }],
          }),
        }),
      );
      const text = await res.text();
    ...
    try {
      const res = await postChatRoute(
        new Request("http://localhost/api/chat", {
          method: "POST",
          body: JSON.stringify({
            // Command-shaped: replayed local context answers commands; natural
            // questions go to the live model when a key is configured.
            message: "today check",
            page_context: {
              surface: "Today",
      ...
      expect(text).toContain("ask before adding risk");
  ...
  test("POST /api/portfolio/import stores read-only wallet holdings separately from manual and sample data", async () => {
    ...
    try {
      ...
      expect(portfolio.daily_review.source_label).toBe("Imported holdings");
      expect(portfolio.daily_review.portfolio_brain_status).toBe("imported");
      expect(portfolio.daily_review.freshness_label).toContain("Imported holdings snapshot");
      expect(portfolio.daily_review.data_boundary).toContain("read-only portfolio context");
      expect(portfolio.daily_review.data_boundary).toContain("cannot place brokerage trades");
