...
// These journeys describe the zero-credential seeded boot, so they must not
// ingest whatever real bundle happens to sit in engine/out on this machine.
let uatPrevEngineDir: string | undefined;

beforeAll(() => {
  uatPrevEngineDir = process.env.ENGINE_OUT_DIR;
  process.env.ENGINE_OUT_DIR = mkdtempSync(join(tmpdir(), "mm-uat-engine-"));
});

afterAll(() => {
  if (uatPrevEngineDir === undefined) delete process.env.ENGINE_OUT_DIR;
  else process.env.ENGINE_OUT_DIR = uatPrevEngineDir;
});
...
describe("UAT user journeys over seeded data", () => {
  test("GIVEN the reviewer seed persona WHEN the app boots with zero credentials THEN every primary surface has seeded backing data", async () => {
    const reviewer = demoDatabase.users.find((user) => user.email === "reviewer@example.test");
    ...
    expect(portfolioPageSubtitle()).toContain("Holdings, allocation, and source status");
    expect(portfolioPageSubtitle()).toContain("Today and chat");
  ...
  test("GIVEN review readiness is required WHEN the review copy is checked THEN it names the seeded local persona and no-login path", () => {
    ...
    expect(reviewer?.role).toBe("reviewer");
    expect(reviewPanelSource).toContain("reviewer@example.test");
    expect(reviewPanelSource).toContain("no password or external login");
    expect(reviewPanelSource).not.toContain(["reviewer", "demo.local"].join("@"));
  });
  ...
  test("GIVEN briefing prompts and quiet states can be sent to live chat WHEN source copy is checked THEN sample data is not described as owned holdings", () => {
    ...
    // Redesign: Today's chat prompt strip was removed with the 90-second brief;
    // the ownership boundary is now the provenance gate on the page label —
    // Today only claims a personal portfolio when holdings are truly personal.
    expect(todayPageSource).toContain(
  ...
  test("GIVEN a manual holding is added WHEN portfolio-aware surfaces reload THEN alerts and chat use the manual portfolio context", async () => {
    ...
    try {
      ...
      // Personal sources replace the demo seed entirely: no sample holdings
      // may remain visible once a manual entry exists.
      expect(portfolioAfterAddApi.holdings.every((holding) => holding.source_label === "Manual entry")).toBe(true);
      expect(portfolioAfterAdd.holdings.every((holding) => holding.source === "manual")).toBe(true);
      expect(portfolioAfterAdd.holdings[0].weight_pct).toBe(100);
      ...
      expect(todayPrompt).toContain("The visible portfolio is built from local manual entries, not imported money");
      expect(todayPrompt).toContain("local manual entry");
      expect(todayPrompt).toContain("built from local manual entries");
      ...
      expect(todayPortfolioScopePhrase(topHolding)).toContain("built from local manual entries");
