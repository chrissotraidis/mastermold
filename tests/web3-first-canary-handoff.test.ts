...
describe("Web3 first canary handoff", () => {
  test("GIVEN first-canary drill and credential requirements WHEN handoff is built THEN it returns one redacted operator packet", () => {
    ...
    expect(receipt.current_step_contract.mode).toBe("web3-first-canary-current-step-contract");
    expect(receipt.current_step_contract.step_id).toBe("wallet-ownership");
    expect(receipt.current_step_contract.can_complete_in_app).toBe(true);
    expect(receipt.current_step_contract.safe_to_provide_now).toContain("hash-only wallet ownership receipt");
    expect(receipt.current_step_contract.never_provide).toContain("private keys");
    expect(receipt.current_step_contract.live_execution_permission).toBe("blocked");
    expect(receipt.proof_ledger.map((item) => item.id)).toEqual(["live-scope", "wallet-ownership", "jupiter-order"]);
    expect(receipt.proof_ledger.find((item) => item.id === "wallet-ownership")?.done).toBe(false);
    expect(receipt.proof_ledger.find((item) => item.id === "live-scope")?.done).toBe(true);
  ...
  test("GIVEN the running route WHEN handoff is requested THEN it composes drill and requirement evidence", async () => {
    ...
    expect(receipt.text_packet).toContain("## Current Step Contract");
    expect(receipt.text_packet).toContain(`- Step id: ${receipt.current_step_contract.step_id}`);
    expect(receipt.current_step_contract?.mode).toBe("web3-first-canary-current-step-contract");
    expect(receipt.current_step_contract?.step_id).toBe(receipt.next_operator_step?.id);
    expect(receipt.current_step_contract?.safe_to_provide_now.length).toBeGreaterThan(0);
    expect(receipt.current_step_contract?.secret_echo_permission).toBe("blocked");
    expect(receipt.proof_ledger?.some((item: { id: string }) => item.id === receipt.current_step_contract.step_id)).toBe(true);
    expect(receipt.private_key_storage).toBe("blocked");
    expect(receipt.secret_echo_permission).toBe("blocked");
  }, 30_000);
});
