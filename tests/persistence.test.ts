...
describe("durable persistence (Phase 1.5)", () => {
  ...
  test("GIVEN a run with news WHEN ingested THEN dated per-symbol facts land in research memory", () => {
    const bundle: EngineBundle = JSON.parse(
      require("node:fs").readFileSync(
        join(process.cwd(), "tests", "fixtures", "engine", "engine-run-active.json"),
        "utf8",
    ...
    bundle.run.news = [
      {
        symbol: "NVDA",
        headline: "Chip demand holds through the quarter",
        publisher: "Test Wire",
    ...
    expect(ingestEngineRun(bundle)).toBe(true);
    const facts = store().marketMemoryFacts();
    const newsFact = facts.find((fact) => fact.topic === "news" && fact.symbol === "NVDA");
    expect(newsFact?.summary).toContain("Chip demand holds through the quarter");
    expect(newsFact?.summary).toContain("Test Wire");
    expect(newsFact?.evidence_urls).toEqual(["https://example.com/nvda"]);
    // Signal/read facts from the same run land alongside the news.
    expect(facts.some((fact) => fact.topic === "market")).toBe(true);
  });
});
