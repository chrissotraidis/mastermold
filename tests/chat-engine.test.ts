...
describe("chat context injection (Phase 3)", () => {
  ...
  test("GIVEN Today is portfolio-aware WHEN local chat answers focus questions THEN it starts with the visible Today focus", async () => {
    process.env.ENGINE_OUT_DIR = FIXTURES;
    const answer = await buildLocalCommandAnswer("What should I focus on today?", {
      surface: "Today",
      route: "/",
      summary: "The user is looking at Today's decision panel.",
    });

    expect(answer?.body).toContain("Today check:");
    expect(answer?.body).toContain("Focus: BTC first");
    expect(answer?.body).toContain("Why:");
    expect(answer?.body).toContain("Risk:");
    expect(answer?.body).toContain("Next: Check Activity");
    expect(answer?.body).not.toContain("Focus: BTC first. NVDA");
    expect(answer?.body).toContain("NVDA moved up 7.1% today");
    const body = answer?.body ?? "";
    expect(body.indexOf("Why:")).toBeLessThan(body.indexOf("Risk:"));
    expect(body.indexOf("Risk:")).toBeLessThan(body.indexOf("Next: Check Activity"));
    expect(answer?.actions[0]).toEqual({ label: "Check Activity", href: "/activity" });
    expect(answer?.body).not.toContain("Focus: NVDA");
    expect(answer?.body).not.toContain("Portfolio:");
    expect(answer?.body).not.toContain("Activity:");
  });

  test("GIVEN Portfolio is checked WHEN local chat answers THEN it uses compact portfolio rows", async () => {
    const answer = await buildLocalCommandAnswer("refresh portfolio balances", {
      surface: "Portfolio",
      route: "/portfolio",
      summary: "The user is reviewing holdings, source, allocation, and portfolio risk.",
    });

    expect(answer?.body).toContain("Portfolio check:");
    expect(answer?.body).toContain("Value:");
    expect(answer?.body).toContain("Exposure:");
    expect(answer?.body).toContain("Source:");
    expect(answer?.body).toContain("Risk:");
    expect(answer?.body).toContain("Next:");
    expect(answer?.body).not.toContain("Data source:");
    expect(answer?.body).not.toContain("Snapshot changes:");
    expect(answer?.body).not.toContain("Review prompt:");
    expect(answer?.actions[0]).toEqual({ label: "Open Portfolio", href: "/portfolio" });
  });

  test("GIVEN Activity is checked WHEN local chat answers THEN it uses compact activity rows", async () => {
    const answer = await buildLocalCommandAnswer("check activity", {
      surface: "Activity",
      route: "/activity",
      summary: "The user is reviewing alerts and recent app activity.",
    });

    expect(answer?.body).toContain("Activity check:");
    expect(answer?.body).toContain("Status:");
    expect(answer?.body).toContain("Top:");
    expect(answer?.body).toContain("Next:");
    expect(answer?.body).not.toContain("Top item:");
    expect(answer?.actions[0]).toEqual({ label: "Open Activity", href: "/activity" });
  });
