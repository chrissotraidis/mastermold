  ...
  return (
    <div className={cn("relative z-20 w-full", className)} data-testid="command-console">
      ...
      <div className="mt-2 grid gap-2">
        ...
        {showSuggestions && typedActions.length === 0 ? (
          <>
            <details className="group sm:hidden">
              <summary className="flex min-h-11 cursor-pointer items-center justify-between rounded-md border border-outline-variant/35 bg-surface-dim/35 px-3 text-xs font-semibold text-on-surface-variant transition hover:border-violet/45 hover:text-on-surface [&::-webkit-details-marker]:hidden">
                Suggestions
                <span aria-hidden="true" className="text-outline transition group-open:rotate-45">+</span>
              </summary>
              <CommandSuggestionButtons suggestions={suggestions} onPick={go} className="mt-2 grid grid-cols-3 gap-2" />
            </details>
            <CommandSuggestionButtons suggestions={suggestions} onPick={go} className="hidden gap-2 sm:flex sm:flex-wrap" />
          </>
        ) : null}
      </div>
    </div>
...
function CommandSuggestionButtons({
  suggestions,
  onPick,
  className,
}: {
