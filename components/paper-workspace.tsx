    ...
    <Card id="paper-trade-form" className="scroll-mt-24 border-outline-variant/40 bg-surface-high/40">
      <CardHeader className="p-5">
        ...
        <p className="text-sm leading-6 text-outline">
          Choose an asset, direction, test amount, and reason. Nothing here places a real trade.
        </p>
      </CardHeader>
...
function PaperAccountPanel({ wallet }: { wallet: PaperWorkspaceData["wallet"] }) {
  return (
    <section
      ...
      className="rounded-lg border border-engine/25 bg-engine/[0.06] p-4 sm:p-5"
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 id="paper-account-title" className="text-xl font-semibold text-on-surface">
            Paper account
          </h2>
      ...
      <p className={wallet.closedResultValue >= 0 ? "mt-3 text-sm text-engine" : "mt-3 text-sm text-critical"}>
      ...
      {activeRound ? (
        <Card className="border-violet/30 bg-violet/[0.055]">
          <CardHeader className="space-y-3 p-5">
            ...
            <CardTitle className="text-2xl text-on-surface">{formatDateRange(activeRound.opensAt, activeRound.closesAt)}</CardTitle>
          </CardHeader>
...
function RoundScorePanel({ round }: { round: PaperWorkspaceData["completedRounds"][number] }) {
  return (
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md border border-outline-variant/40 bg-surface-dim/45 p-3">
      ...
      <p className="mt-1 break-words text-base font-semibold text-on-surface sm:text-lg">{value}</p>
    </div>
  );
}
