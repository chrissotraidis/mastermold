      ...
      <div className="mt-5">
        {holdings.length > 0 ? (
          <details className="rounded-md border border-outline-variant/25">
            <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-3 px-3 py-2 text-sm font-semibold text-on-surface marker:hidden [&::-webkit-details-marker]:hidden">
              <span>
                {holdings.length} saved manual holding{holdings.length === 1 ? "" : "s"}
              </span>
              <span className="text-xs font-normal text-outline">Open to edit or remove</span>
            </summary>
            <div className="divide-y divide-outline-variant/15 border-t border-outline-variant/25">
                ...
                >
                  <p className="min-w-0 truncate text-sm text-on-surface">
                    ...
                    <span className="text-on-surface-variant">
                      {" "}
                      · {formatCurrency(holding.market_value)} · {formatQuantity(holding.quantity)} held ·{" "}
                      {holding.weight_pct.toFixed(1)}% of visible portfolio
                    </span>
                  </p>
                  <button
                    type="button"
                    ...
                    className="inline-flex min-h-11 shrink-0 items-center gap-1.5 rounded-md px-2 text-xs text-on-surface-variant transition hover:text-critical disabled:opacity-50"
                  >
