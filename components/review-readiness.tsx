...
const disclosureSections = [
  {
    ...
    items: [
      ...
      "Trading now opens with a compact live trading command center that combines live ignition, first-canary readiness, the next safe credential/blocker, and the proof watcher command before the canary console, while detailed ignition and canary-readiness receipts sit in the review drawer.",
      "A Node-only npm run drill-canary:web3 command now consolidates the first-canary runway across live blockers, supervised canary readiness, Jupiter order proof, unsigned-order preflight, and canary proof receipts while staying read-only and unable to sign, submit, or move funds.",
      ...
      "/api/web3-live-trade-canary, /api/web3-live-unsigned-order-handoff, /api/web3-wallet-ownership, Trading, and Settings now explicitly state whether a real live trade has actually been tested, separate paper/read-only/Jupiter rehearsal from money movement, let Trading preflight the exact canary wallet before any prompt, let Trading prove wallet ownership with a server-issued text-only challenge and hash receipt, show a canary launch checklist for preflight, wallet signing, relay, and confirmation/accounting, show the post-signing proof chain for signed relay, chain confirmation, settlement reconciliation, and portfolio mirror accounting, and let the live Trading canary run a bounded auto-watch proof loop after a wallet signature.",
