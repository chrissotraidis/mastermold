...
const disclosureSections = [
  {
    ...
    items: [
      ...
      "A live-capital preflight drill now combines live-readiness state, daemon handoff real-capital boundaries, and repeat-proof promotion gates, returning the current live blocker quickly when wallet/signing gates are already closed, bounding forced repeat proof with a timeout, and failing closed unless any future live-ready state is explicitly allowed for manual executor review.",
      ...
      "The Web3 trading command center now includes an actual live-trade test ledger, also exposed as /api/web3-live-test-ledger, that separates paper autonomy, read-only live DEX reads, Jupiter/order rehearsal, live flag arming, and funded-wallet trade proof so paper profit and rehearsals are not mistaken for real money movement.",
      "/api/web3-live-usability-summary now combines the live usability blockers, local credential install health, and actual live-test ledger into one compact not-yet-usable receipt for funded autonomous trading, while still blocking live execution, signing, transaction submission, wallet mutation, private-key storage, seed-phrase storage, and secret echo.",
      "/api/web3-live-usability-summary now builds the blocker-backed receipt by default, so the first safe input, real-capital blocker count, live-usability row count, and actionable Trading/Settings surface are visible unless the deeper packet times out or compact=1 is explicitly requested.",
      "/api/health now builds one shared canonical live-dex persistent state for the live first-canary drill, live canary proof, and first live-canary attempt summaries, reducing repeated provider work while preserving the same blocked live-execution, submission, signing, wallet-mutation, and no-funded-trade claims.",
      "Slow live-market canary receipts now fail closed without replacing the operator next action with a timeout string: the ledger and compact live-usability fallback still point at the dedicated public-wallet step while keeping live execution, signing, submission, and wallet mutation blocked.",
      "/api/web3-dedicated-wallet-intake-contract now accepts source and defaults to the live DEX setup path, so Trading, Settings, smoke checks, and helper receipts inspect the same live-canary wallet contract instead of a sample-scoped contract.",
      "/api/web3-dedicated-wallet-intake-contract now exposes the exact safe next wallet input contract: existing dry-run scope save endpoint, public wallet placeholder, accepted non-secret cap fields, rejected wallet-secret and transaction fields, verifier command, and blocked live/signing/submission/wallet authority.",
      "/api/web3-dedicated-wallet-intake-contract now supports validate-only POST for proposed public wallet scope, returning redacted wallet preview, field-path validation, sample-wallet rejection, cap blockers, and the next proof runway without saving state, signing, submitting, mutating wallets, or echoing secrets.",
      "A Node-only npm run validate-wallet:web3 command now wraps that validate-only wallet intake receipt, refusing unsafe credential-shaped flags, failing if the full wallet is echoed, printing the next proof runway, and keeping live execution, transaction submission, signing, and wallet mutation blocked.",
      "Settings now renders that dedicated wallet intake contract beside the wallet packet, showing accepted fields, rejected fields, dry-run save body, after-save verifier steps, the contract JSON link, and the same live/signing/submission/wallet-secret blocks before the operator reaches credential fields.",
      ...
      "Settings now includes a dedicated wallet packet that rejects invalid or sample all-ones wallet scope for live-review readiness, asks for a public Solana address only, points the canonical collection action back to the Trading live canary console, points at text-only wallet ownership proof, and exposes the strict --wallet=<public-solana-address> operator-wallet verifier while live execution and wallet mutation stay blocked.",
      ...
      "Trading can now record a redacted live-canary gate-check receipt through /api/web3-supervised-canary-readiness without signing, submitting, or storing transaction data, so the app can show the latest checked blocker while still reporting that no funded trade has happened until the canary proof chain is real.",
      ...
      "Trading now adds a first canary attempt summary above the live-test ledger, showing runnable state, current stage, missing input count, acknowledgement count, exact next command, next endpoint, readiness JSON, and setup runway while live execution, signing, submission, and wallet mutation remain blocked.",
      ...
      "Trading live canary console now has a dedicated public-wallet scope panel that can save only a public Solana wallet and dry-run caps, rejects the demo all-ones wallet for funded canary scope, refreshes canary receipts, and still cannot sign, submit, store wallet authority, mutate funds, or count as a live trade.",
      ...
      "Trading and Settings now surface wallet proof freshness for the first funded canary, including proof age, expiry, and the fresh/stale distinction, so a verified-but-expired hash receipt cannot be mistaken for canary-ready wallet authorization.",
      "Settings now adds a first-canary safe prep queue that separates do-now public/env setup from wait, external wallet, and post-signing proof steps, so Jupiter/live-flag prep can happen without implying the wallet proof gate has been cleared.",
      ...
      "/api/web3-live-trade-canary and the Trading canary console now expose structured required_inputs plus next_required_input for dedicated public wallet scope, wallet ownership proof, Jupiter order rail, first-canary live flags, unsigned-order preflight, signed-payload relay, and post-signing proof, with safe surfaces, target names, verifier commands, completion signals, and blocked live execution, transaction submission, wallet mutation, and secret echo on every row.",
      ...
      "/api/web3-live-trade-canary now checks signed-payload request and submit-route continuity before attempting relay, orders canary blockers by live-scope, wallet, ownership-proof, Jupiter, live-flag, and request-id prerequisites before final signature proof, reports the expected current request id and route, keeps relayed-but-unconfirmed signatures out of actual_live_trade_tested, marks stale, route-mismatched, or missing canary request ids as blocked, and still never echoes signed payloads, private keys, seed phrases, API keys, raw transaction fields, wallet authority, or live execution permission.",
      ...
      "A Node-only npm run credentials-local:web3 command now reads the redacted localhost Web3 local credential status with GET only, reports configured and missing env target names, refuses unsafe secret-shaped flags, and verifies live execution, wallet mutation, and secret echo remain blocked.",
      "A Node-only npm run status-canary:web3 command, /api/web3-canary-status, and the Trading Running app canary status panel now reconcile the running app's live canary, ignition, and local credential receipts with GET only, report whether a funded live trade was actually tested, expose gate_progression plus safe_next_commands for wallet scope/proof/status follow-up, fail on contradictory next-gate guidance, refuse unsafe secret-shaped flags, and keep signing, submission, wallet mutation, private-key storage, seed-phrase storage, and secret echo blocked.",
      "A Node-only npm run scope-wallet:web3 command now validates a public Solana wallet by default, saves that public scope only with --save, refreshes /api/web3-canary-status afterward, refuses private-key/seed/signed-payload/API-key-shaped flags, and keeps live execution, transaction submission, wallet mutation, signing authority, private-key storage, seed-phrase storage, and secret echo blocked.",
      "A Node-only npm run prove-wallet:web3 command now fetches the text-only /api/web3-wallet-ownership challenge for a public wallet, submits an external browser-wallet message signature, returns hash-only proof fields, refuses private-key/seed/transaction/API-key-shaped flags, and keeps live execution, transaction signing, transaction submission, wallet mutation, custody, private-key storage, seed-phrase storage, and secret echo blocked.",
      ...
      "Settings now checks the redacted local Web3 credential status with a read-only GET on load and through Check server status, so configured and missing env target names are visible before any credential values are posted.",
      ...
      "The Web3 trading cockpit now shows a first-screen operator input packet driven by the live canary receipt, naming the next safe public/env/proof input, owner, safe surface, verifier command, open canary input rows, and never-paste boundary before the full canary console and long workspace; it also builds the canonical first-canary handoff so the redacted current-step contract, proof ledger, and local handoff-canary export command can be copied from Trading.",
  ...
  },
  {
    ...
    items: [
      ...
      "Trading can save the first-canary public wallet scope from the live canary console, but that write is public scope only and does not prove ownership, sign a transaction, submit a swap, or authorize autonomous real-capital trading.",
