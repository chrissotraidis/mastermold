...
const disclosureSections = [
  {
    ...
    items: [
      ...
      "Signer ops compares external wallet prompts, Privy server-wallet, Turnkey policy-wallet, and session-key-vault paths, then exposes the active provider's signature request, auto-sign, user-presence, payload, expiry, cap, and live-gate readiness.",
      ...
      "The Web3 autonomous forward suite now repeats the bounded paper-daemon proof across base, breakout, and rug-risk sample regimes, reporting aggregate PnL, moved/traded regime counts, hot-coin baseline alpha, and best/worst scenario outcomes so the make-money loop is not judged from one tape only.",
  ...
  },
  {
    ...
    items: [
      ...
      "Signer ops results are provider readiness metadata. They can say whether a wallet prompt, Privy-style server wallet, Turnkey-style policy wallet, or session-key path is configured enough to request a bounded signature, but they do not store secrets, raw transaction bodies, or signed payloads.",
      "Autonomous live-readiness audit is a conservative transition checklist only. In the current build it keeps real-capital autonomy locked unless every live gate clears; it still does not provide a durable background worker, custody provider, signer policy enforcement, tax/compliance approval, or guaranteed profit.",
      "Autonomous daemon handoff is a scheduler contract only. It can tell an external runner when and how to call the paper backend loop, but it does not itself install a daemon, prevent every overlapping process outside the app, sign transactions, submit swaps, or keep running after hosting/runtime shutdown.",
  ...
  },
  {
    ...
    items: [
      ...
      "Signer ops provider, policy hash, wallet scope, request count, expiry, auto-sign, and user-presence values are local readiness metadata until real provider credentials and explicit live-gate approvals are configured.",
  ...
  },
  {
    ...
    items: [
      ...
      "Signer ops actions can request a wallet prompt or mark a provider policy path as ready only inside local checks. They cannot bypass provider policy, create external wallets, or sign without a configured signer.",
      ...
      "Execution retry planning, preflight, transaction lifecycle tracking, signed-relay attempts, Jupiter Trigger create/history/reconcile drills, and local portfolio-mirror patches record route, fee, quote age, quote timestamp, context slot, nonce, request id, retry-window, blocker metadata, payload hash, byte count, lifecycle stage, signature/status metadata, fill estimates, applied order ids, and synthetic signer proof locally. They do not store transaction bodies, sign with real keys, or accept private keys.",
