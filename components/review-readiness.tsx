...
const disclosureSections = [
  {
    ...
    items: [
      ...
      "/api/web3-credential-requirements now exposes that safe credential checklist as standalone JSON, and /api/health exposes compact web3_credential_requirements counts, hashes, next requirement, source endpoints, and blocked live/signing/wallet/secret permissions for monitors.",
      ...
      "/api/web3-live-activation-plan, npm run activate:web3, Settings, and /api/health now expose one consolidated Web3 live activation plan with ordered milestones, next safe milestone, strict verifier commands, copyable redacted Markdown, and blocked activation/signing/submission/wallet authority.",
      ...
      "/api/web3-live-ignition, Trading, /api/health, and npm run verify:web3 now expose one bot-facing real-money ignition go/no-go receipt plus a POST launch-envelope action that reconciles live scope, wallet scope, route/order proof, signer relay, autonomy gate, canary proof, and the safety boundary while can_autonomously_trade_real_money_now remains false until a funded canary is relayed, confirmed, reconciled, and mirrored.",
      "/api/web3-supervised-canary-readiness, Trading, and npm run verify:web3 now expose the first-funded-canary ladder, separating the tiny unsigned-order request, external browser-wallet signature, guarded signed-payload relay, manual review, and funded canary proof while live execution and wallet mutation remain blocked until the proof chain is real.",
      "Trading now opens with a compact live trading command center driven by the first-canary drill receipt: drill status, hash, hard-fail count, proof count, live-dex drill link, ordered next lane, leading failing lanes, next safe credential/blocker, next post-signing proof step, signature preview, and proof watcher command appear before the canary console, while detailed ignition and canary-readiness receipts sit in the review drawer.",
      "GET /api/web3-first-canary-drill, /api/health, and the Node-only npm run drill-canary:web3 command now consolidate the first-canary runway across live blockers, supervised canary readiness, Jupiter order proof, unsigned-order preflight, and canary proof receipts while staying read-only and unable to sign, submit, or move funds; /api/health also exposes web3_live_first_canary_drill as the canonical live-dex persistent monitor summary, web3_live_canary_proof as the compact post-signing proof monitor, and the drill health separates the ordered canary lane from the broader credential-intake label.",
      "/api/web3-live-trade-canary now checks signed-payload request continuity before attempting relay, reports the expected current request id, keeps relayed-but-unconfirmed signatures out of actual_live_trade_tested, marks stale or missing canary request ids as blocked, and still never echoes signed payloads, private keys, seed phrases, API keys, raw transaction fields, wallet authority, or live execution permission.",
      "/api/web3-live-unsigned-order-handoff now records a redacted continuity audit when it returns a one-shot canary order, preserving only request id, route, payload hash, and byte count so the signed-payload relay can match the browser-wallet signature without storing unsigned transaction bytes.",
      "/api/web3-live-trade-canary, /api/web3-live-unsigned-order-handoff, /api/web3-wallet-ownership, Trading, and Settings now explicitly state whether a real live trade has actually been tested, separate paper/read-only/Jupiter rehearsal from money movement, let Trading preflight the exact canary wallet before any prompt, let Trading prove wallet ownership with a server-issued text-only challenge and hash receipt, show a canary launch checklist for preflight, wallet signing, relay, and confirmation/accounting, show the post-signing proof chain for signed relay, chain confirmation, settlement reconciliation, and portfolio mirror accounting in Trading and Settings, and let the live Trading canary run a bounded auto-watch proof loop after a wallet signature.",
      ...
      "A Node-only npm run requirements:web3 command now prints the standalone Web3 credential requirements packet as Markdown or JSON, checks the next wallet requirement and blocked live/signing/wallet/secret permissions, and redacts configured local provider secrets from failures.",
      ...
      "The Web3 trading header and provenance chip now switch from Sample to Live DEX read when the cockpit is opened with `source=live-dex`, keeping public DEX evidence distinct from seeded review tape.",
