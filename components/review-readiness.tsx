...
const disclosureSections = [
  {
    ...
    items: [
      ...
      "Settings and /api/web3-operator-request-packet now expose a shareable redacted Web3 request packet with required inputs, safe-to-provide values, never-provide values, verifier commands, and blocked live-authority boundaries.",
      ...
      "Settings now includes a dedicated wallet packet that rejects the sample all-ones wallet for live-review readiness, asks for a public Solana address only, points at text-only wallet ownership proof, and exposes the strict operator-wallet verifier while live execution and wallet mutation stay blocked.",
      ...
      "Settings now includes a Web3 research handoff packet and /api/web3-research-handoff-packet so provider, custody, risk, operations, product, and profit-proof questions can be safely sent to another helper without secrets, transaction bytes, signing authority, wallet mutation, or live execution.",
      ...
      "Settings now includes a Web3 research answer intake console backed by /api/web3-research-answer-intake so redacted helper answers can be scored into custody, provider, risk, ops, cockpit, and profit-proof decision coverage while secret-looking pasted values are rejected and live authority stays blocked.",
      "/api/health now exposes a compact Web3 research handoff health summary with question counts, open-input counts, live-blocker counts, and the same live-execution, wallet-mutation, and secret-echo locks.",
      ...
      "The Web3 trading cockpit now opens with a command board that combines the current autonomous decision, paper wallet net-worth curve, next usable gate, supervised-live lane count, missing operator inputs, signal score, and route proof before the deeper receipt stack.",
      ...
      "The Web3 trading page and /api/web3-operator-runbook now expose a safe operator action map that says which paper, read-only, credential, verifier, and external-review actions can run now while real-capital autonomous trading stays blocked.",
      ...
      "A Node-only npm run verify:web3 gate now snapshots and restores the saved public wallet/risk scope while checking the running app's Web3 health receipts, malformed wallet rejection, private-field rejection, public dry-run wallet scope save, validate-only credential redaction, configured Helius/Solana provider-health proof, manual live-review packet boundaries, deterministic DEX discovery receipt boundaries, one-shot Jupiter rehearsal redaction, and live-execution/wallet-mutation locks for machines where Bun tests are unavailable.",
