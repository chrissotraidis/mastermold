...
const NAV: NavItem[] = [
  ...
  { href: "/trading", label: "Trade", icon: Bot, zone: "act" },
  { href: "/activity", label: "Activity", icon: Activity, zone: "observe" },
  { href: "/settings", label: "Settings", icon: Settings, zone: "system" },
];

const WARM_ROUTES = ["/", "/portfolio", "/trading", "/activity", "/settings", "/chat"] as const;
...
export function AppShell({
  ...
  return (
    <div className="relative min-h-screen scanline-bg">
      ...
      />
      <SideRail />
      <main className="mx-auto w-full max-w-deck px-margin-mobile pb-[calc(8rem+env(safe-area-inset-bottom))] pt-20 md:pl-24 md:pr-margin-desktop md:pb-12">
        {children}
      </main>
      <MobileNav />
      <GlobalAssistant />
...
function labelAppShellCommandAction(action: string) {
  if (action === "run-paper-test") return "Run Paper Test";
  ...
  if (action === "import-portfolio-snapshot") return "Import Holdings Snapshot";
...
function TopBarFallback({ dataMode }: { dataMode: DataModeLabel }) {
  ...
  return (
    <header className="fixed top-0 left-0 z-50 flex h-16 w-full items-center justify-between border-b border-outline-variant/60 bg-surface-dim/80 px-margin-mobile backdrop-blur-xl md:px-margin-desktop">
      ...
      <div className="h-10 w-10 rounded-md border border-outline-variant/40 bg-surface-high/40" aria-hidden="true" />
    </header>
  );
}
    ...
    <header className="fixed top-0 left-0 z-50 flex h-16 w-full items-center justify-between border-b border-outline-variant/60 bg-surface-dim/80 px-margin-mobile backdrop-blur-xl md:px-margin-desktop">
      <div className="flex min-w-0 items-center gap-3">
        ...
        <button
          ...
          className="flex min-h-11 items-center font-display text-2xl font-bold tracking-tighter text-violet transition hover:text-tertiary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet md:hidden"
          aria-label="Ask Master Mold from the top bar"
          title="Ask Master Mold"
        >
          Master Mold
        </button>
        <Link href="/" className="hidden min-h-11 items-center font-display text-2xl font-bold tracking-tighter text-violet md:flex">
          Master Mold
        </Link>
        <span
          className={cn(
      ...
      <div className="flex items-center gap-2">
        {showCommandBar ? (
          <div className="group relative hidden lg:block">
            ...
            >
              <div className="flex min-h-12 items-center gap-2 rounded-md border border-violet/35 bg-surface-low/90 px-3 py-1.5 shadow-sm shadow-void/15 transition-colors focus-within:border-violet focus-within:ring-2 focus-within:ring-violet/30">
                ...
                <input
                  ...
                  className="min-h-11 w-56 bg-transparent text-sm font-medium text-on-surface placeholder:text-outline focus:outline-none xl:w-72"
                />
                <button
                  type="submit"
...
function pageContextForShellPath(pathname: string, route = pathname): ChatPageContext {
  ...
  if (pathname.startsWith("/settings")) {
    return {
      ...
      route,
      summary:
        "The user is looking at connection checks, manual holdings, holdings snapshot imports, AI/chat keys, Web3 wallet setup, safety limits, and privacy.",
    };
  }
              ...
              <NavPendingState>
                {(pending) => (
                  <>
                    ...
                    <span
                      className={cn(
                        "hidden font-mono text-[12px] uppercase tracking-telemetry group-hover:inline-block",
...
const MOBILE: Array<NavItem & { shortLabel: string }> = [
  ...
  { href: "/trading", label: "Trade", shortLabel: "Trade", icon: Bot, zone: "act" },
  { href: "/activity", label: "Activity", shortLabel: "Activity", icon: Activity, zone: "observe" },
...
export function FirstRunBanner() {
  return (
    <div className="mb-gutter flex items-start gap-3 border border-outline-variant/40 bg-surface-dim/50 p-4 chamfer-sm backdrop-blur-sm inner-glow">
      ...
      >
        <ArrowLeft className="size-3.5 rotate-180" /> System status
      </Link>
    </div>
  );
}
