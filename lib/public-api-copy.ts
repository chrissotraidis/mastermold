...
export type PublicPortfolioRecommendation = {
  id: string;
  symbol: string;
  classification: "Review" | "Watch" | "Trim candidate" | "Add candidate" | "Paper test first";
  title: string;
  detail: string;
  reason: string;
  href: string;
...
export type PublicPortfolioDailyReview = {
  source_label: string;
  portfolio_brain_status: "not_synced" | "ready" | "partial" | "replay" | "fresh" | "stale" | "manual" | "imported" | "sample";
  freshness_label: string;
  read_only: true;
  synced_at: string | null;
  as_of: string | null;
  data_boundary: string;
  recommendations: PublicPortfolioRecommendation[];
};
...
export type PublicPortfolio = {
  ...
  daily_review: PublicPortfolioDailyReview;
...
export function toPublicPortfolio(
  ...
  dailyReview: PublicPortfolioDailyReview = emptyPublicPortfolioDailyReview(),
): PublicPortfolio {
  ...
  return {
    summary: {
      ...
      source_note: publicPortfolioSourceNote(portfolio.provenance.label, portfolio.provenance.source),
    },
    holdings,
    ...
    provenance: {
      ...
      source: publicPortfolioSourceNote(portfolio.provenance.label, portfolio.provenance.source),
...
function emptyPublicPortfolioDailyReview(): PublicPortfolioDailyReview {
  return {
    source_label: "Visible portfolio",
    portfolio_brain_status: "not_synced",
    freshness_label: "No portfolio review context loaded",
    read_only: true,
    synced_at: null,
    as_of: null,
    data_boundary: "Review prompt only. This cannot place trades, sign transactions, or move funds.",
    recommendations: [],
  };
}
...
function publicPortfolioSourceNote(label: PortfolioJson["provenance"]["label"], source = "") {
  if (label === "Imported portfolio") {
    if (source.includes("Monarch MCP")) {
      return "Monarch MCP portfolio brain snapshot plus local manual holdings when available.";
    }
    return "Imported holdings snapshots plus local manual entries when available.";
  }
  if (label === "Manual portfolio") {
    return "Local manual entries.";
  }
