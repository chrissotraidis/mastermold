export function plainBriefingText(value: string) {
  return value
    ...
    .replace(
      /\baUSDC is a DeFi asset that may appeal to investors seeking yield opportunities in stablecoin markets\.?/gi,
      "aUSDC is a small on-chain cash position; review it only if you planned to adjust that cash.",
    )
    .replace(
