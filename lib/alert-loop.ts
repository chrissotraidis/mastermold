...
export function explainAlertRelevance(alert: AlertLoopInput) {
  ...
  const tier = alertTier(alert);
  const symbol = alertSubjectSymbol(alert);
  const relevance =
    portfolioWeight > 0
...
export function buildAlertSuggestedResponse(alert: AlertLoopInput) {
  ...
  const tier = alertTier(alert);
  const subject = alertSubjectSymbol(alert) ?? cleanAlertMessage(alert.message).split(" ")[0] ?? "This";
...
/**
 * The alert's real ticker, or undefined when the payload carries no usable
 * symbol. Older saved alerts (and demo-asset misses before the symbol
 * resolution fix in src/db/alerts.ts) serialized the literal placeholder
 * "Unknown"; never let that word reach user-facing copy as a ticker.
 */
function alertSubjectSymbol(alert: AlertLoopInput): string | undefined {
  for (const candidate of [alert.asset_symbol, alert.asset_key]) {
    const trimmed = candidate?.trim();
    if (trimmed && trimmed.toLowerCase() !== "unknown" && trimmed.toLowerCase() !== "unknown asset") {
      return trimmed;
    }
  }
  return undefined;
...
function buildAlertFalsification(alert: AlertLoopInput) {
  const subject = alertSubjectSymbol(alert) || cleanAlertMessage(alert.message).split(":")[0] || "This activity item";
