...
export function todayMorningSummary(
  ...
  const alertText = openAlertCount > 0 ? `${openAlertCount} alert${openAlertCount > 1 ? "s" : ""} to review.` : "No open alerts.";
...
export function buildTodayPrompt(
  ...
  return [
    ...
    topAlert
      ? `Top alert to also check: ${cleanAlertMessage(topAlert.message)}. ${todayAlertPromptRelevance(topAlert, topHolding)}.`
      : "No open alert is visible.",
    "Tell me the 1-3 things to focus on, in the same order as the visible Today page: Focus 1 first, then urgent alerts or smaller checks. Explain why each matters to the visible portfolio and what action is reasonable. Do not use jargon without translating it.",
  ].join(" ");
}
...
export function todayHoldingPromptDetail(holding: TodayHolding | null) {
  ...
  if (holding.source === "manual") {
    return `The visible portfolio includes local manual entries plus sample data. Its largest visible holding is ${holding.symbol} at ${holdingWeight} from a local manual entry.`;
  }
...
export function todayPortfolioScopePhrase(holding: TodayHolding | null) {
  ...
  if (holding.source === "manual") {
    return "the visible portfolio, including local manual entries plus sample data";
  }
...
export function buildTodayRiskNote({
  ...
  if (activeAlerts > 0) {
    return `${activeAlerts} alert${activeAlerts > 1 ? "s" : ""} are open. Review those before adding a new idea.`;
  }
  if (highScored > 0) {
