...
from . import adapter, data_cache, screener
...
def assemble_run(
    *,
    ...
    run_meta = {
        ...
        "watchlist_source": config.get("watchlist_source", "config"),
...
def main(argv: list[str] | None = None) -> int:  # pragma: no cover
    ...
    watchlist_override_path = os.environ.get("ENGINE_WATCHLIST_PATH")
    if watchlist_override_path:
        override_entries = load_watchlist_override(Path(watchlist_override_path))
        if override_entries:
    ...
    bundle["run"]["stages"]["data_refresh"] = data_refresh_status
    bundle["run"]["stages"]["data_refresh_detail"] = data_refresh_detail
    bundle["run"]["stages"]["agent_adapter"] = adapter_status
    bundle["run"]["news"] = gather_news(config["watchlist"], triggered)
    bundle["run"]["stages"]["news"] = f"{len(bundle['run']['news'])} headlines"
...
def load_market_signals(
    run_date: str,
    watchlist: Sequence[dict[str, Any]],
) -> tuple[dict[str, dict[str, Sequence[float]]], str, dict[str, Any]]:
    """Fetch one boring real source when possible, then visibly fall back per symbol."""
    fallback = synthetic_signals(watchlist)
    signals: dict[str, dict[str, Sequence[float]]] = {}
    ...
    for entry in watchlist:
        ...
        yf_symbol = yf_symbol_for_entry(entry)
        if not yf_symbol:
            signals[symbol] = fallback[symbol]
            fallback_symbols.append(symbol)
            errors[symbol] = "No yfinance symbol configured."
            continue
        try:
            payload = data_cache.refresh_ticker(run_date, yf_symbol)
            bar_count = len(payload.get("bars", []))
            news_counts = [0.0] * bar_count
            derived = data_cache.screener_signals(payload, news_counts)
            if min((len(series) for series in derived.values()), default=0) < 3:
                raise RuntimeError("Not enough cached bars for screener.")
            signals[symbol] = derived
            real_symbols.append(symbol)
        except Exception as exc:
            signals[symbol] = fallback[symbol]
            fallback_symbols.append(symbol)
            errors[symbol] = f"{type(exc).__name__}: {str(exc)[:140]}"

    if real_symbols and fallback_symbols:
        status = "yfinance_partial"
    elif real_symbols:
        status = "yfinance"
    else:
        status = "synthetic_signals"

    return signals, status, {
        "source": "yfinance" if real_symbols else "synthetic fallback",
...
def gather_news(
    ...
    """Free news stage: top yfinance headlines for the triggered tickers only.

    Headlines land in run meta so the dashboard's research memory can persist
    them as dated per-symbol facts. Best-effort by design — a failed fetch for
    one symbol never fails the run.
    """
    ...
    entries_by_symbol = {str(e.get("symbol")): e for e in watchlist}
    collected: list[dict[str, Any]] = []
    for symbol in list(triggered)[:8]:
        entry = entries_by_symbol.get(symbol)
        yf_symbol = yf_symbol_for_entry(entry) if entry else None
        if not yf_symbol:
            continue
        ...
        for item in raw_items:
            if count >= per_symbol:
                break
            # yfinance has shipped both flat items and {"content": {...}} shapes.
            content = item.get("content") if isinstance(item.get("content"), dict) else item
            title = str(content.get("title") or "").strip()
            if not title:
                continue
            provider = content.get("provider") if isinstance(content.get("provider"), dict) else {}
            url_field = content.get("canonicalUrl") if isinstance(content.get("canonicalUrl"), dict) else {}
            collected.append(
                {
                    "symbol": symbol,
                    "headline": title[:300],
                    "publisher": str(provider.get("displayName") or content.get("publisher") or "")[:80],
                    "url": str(url_field.get("url") or content.get("link") or "")[:500],
                    "published_at": str(content.get("pubDate") or content.get("providerPublishTime") or ""),
                }
            )
            count += 1
    return collected
...
def load_watchlist_override(path: Path) -> list[dict[str, Any]] | None:
    """Watchlist injected by the dashboard from the operator's live portfolio.

    The app writes this file before each scan so the engine screens what the
    operator actually holds instead of the demo config list. Malformed files
    fall back to the config watchlist rather than failing the run.
    """
    try:
    ...
    for item in raw:
        ...
        if not symbol or len(symbol) > 12 or asset_class not in {"equity", "crypto"}:
            continue
        entry: dict[str, Any] = {
            "symbol": symbol,
            "asset_id": str(item.get("asset_id") or f"asset_{symbol.lower()}"),
            "asset_class": asset_class,
        }
        if item.get("yf_symbol"):
            entry["yf_symbol"] = str(item["yf_symbol"])
        entries.append(entry)
    return entries or None


def yf_symbol_for_entry(entry: dict[str, Any]) -> str | None:
    if entry.get("yf_symbol"):
        return str(entry["yf_symbol"])
