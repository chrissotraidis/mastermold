...
async function main() {
  ...
  assert(health.web3_operator_runbook?.mode === "web3-operator-runbook-health", "Health endpoint should expose Web3 operator runbook health.", health.web3_operator_runbook);
  assert(["setup-needed", "paper-operable", "supervised-review-ready"].includes(health.web3_operator_runbook.status), "Operator runbook health should expose a known status.", health.web3_operator_runbook);
  assert(typeof health.web3_operator_runbook.receipt_hash === "string" && health.web3_operator_runbook.receipt_hash.length === 64, "Operator runbook health should expose a receipt hash.", health.web3_operator_runbook);
  ...
  assert(health.web3_operator_runbook.current_input?.live_execution_permission === "blocked", "Operator runbook health should expose a locked current input contract.", health.web3_operator_runbook);
  assert(Array.isArray(health.web3_operator_runbook.current_input.target_names), "Operator runbook health current input should expose safe target names.", health.web3_operator_runbook.current_input);
  assert(health.web3_operator_runbook.live_execution_permission === "blocked", "Operator runbook health should keep live execution blocked.", health.web3_operator_runbook);
  assert(health.web3_operator_runbook.transaction_submission_permission === "blocked", "Operator runbook health should keep transaction submission blocked.", health.web3_operator_runbook);
  assert(health.web3_operator_runbook.wallet_mutation_permission === "blocked", "Operator runbook health should keep wallet mutation blocked.", health.web3_operator_runbook);
  assert(health.web3_operator_runbook.signing_permission === "blocked", "Operator runbook health should keep signing blocked.", health.web3_operator_runbook);
  assert(health.web3_operator_runbook.private_key_storage === "blocked", "Operator runbook health should keep private-key storage blocked.", health.web3_operator_runbook);
  assert(health.web3_operator_runbook.seed_phrase_storage === "blocked", "Operator runbook health should keep seed-phrase storage blocked.", health.web3_operator_runbook);
  assert(health.web3_operator_runbook.secret_echo_permission === "blocked", "Operator runbook health should keep secret echo blocked.", health.web3_operator_runbook);
  ...
  assert(health.web3_live_usability.next_credential_request.verification_runway.every((step) => step.live_execution_permission === "blocked" && step.wallet_mutation_permission === "blocked" && step.secret_echo_permission === "blocked"), "Live-usability health verification runway should keep live, wallet, and secret permissions blocked.", health.web3_live_usability.next_credential_request);
  assert(health.web3_live_usability.next_credential_request?.live_execution_permission === "blocked", "Live-usability health next credential request should keep live execution blocked.", health.web3_live_usability.next_credential_request);
  assert(health.web3_live_usability.next_credential_request?.secret_echo_permission === "blocked", "Live-usability health next credential request should keep secret echo blocked.", health.web3_live_usability.next_credential_request);
  ...
  assert(["missing", "partial", "ready"].includes(launchChecklist.provider_credentials_readiness.read_provider_status), "Provider credential readiness should expose the read-provider rail status.", launchChecklist.provider_credentials_readiness);
  assert(launchChecklist.provider_credentials_readiness.checks.some((check) => check.id === "read-provider-rail"), "Provider credential readiness should include Helius/Jupiter read-provider rail evidence.", launchChecklist.provider_credentials_readiness);
  ...
  assert(["complete", "refresh-first", "preflight-repair", "run-paper-session", "protect-first", "blocked"].includes(profitAccountability.repair_plan.status), "Autonomous profit accountability repair should expose a known status.", profitAccountability.repair_plan);
  ...
  assert(typeof profitAccountability.repair_plan.local_route_rehearsal_ready === "boolean", "Autonomous profit accountability repair should disclose whether sample route rehearsal was accepted.", profitAccountability.repair_plan);
  assert(profitAccountability.repair_plan.local_route_rehearsal_summary === null || typeof profitAccountability.repair_plan.local_route_rehearsal_summary === "string", "Autonomous profit accountability repair should summarize accepted sample route rehearsal.", profitAccountability.repair_plan);
  assert(profitAccountability.repair_plan.blocking_reason === null || typeof profitAccountability.repair_plan.blocking_reason === "string", "Autonomous profit accountability repair should disclose the remaining blocker after route proof.", profitAccountability.repair_plan);
  ...
  assert(["not-configured", "ready", "skipped", "blocked"].includes(orderRehearsal.payload.wallet_holdings_adapter?.asset_index_status), "Wallet holdings adapter should expose Helius DAS asset-index status.", orderRehearsal.payload.wallet_holdings_adapter);
  assert(orderRehearsal.payload.wallet_activity_history?.mode === "read-only-wallet-activity-history", "Order rehearsal should expose read-only wallet activity history.", orderRehearsal.payload.wallet_activity_history);
  assert(["missing-wallet", "not-configured", "ready", "empty", "blocked"].includes(orderRehearsal.payload.wallet_activity_history?.status), "Wallet activity history should return a known status.", orderRehearsal.payload.wallet_activity_history);
  assert(orderRehearsal.payload.wallet_activity_history?.live_execution_permission === "blocked", "Wallet activity history should not unlock live execution.", orderRehearsal.payload.wallet_activity_history);
  assert(orderRehearsal.payload.wallet_activity_history?.wallet_mutation_permission === "blocked", "Wallet activity history should keep wallet mutation blocked.", orderRehearsal.payload.wallet_activity_history);
  assert(orderRehearsal.payload.wallet_transaction_intelligence?.mode === "read-only-wallet-transaction-intelligence", "Order rehearsal should expose read-only wallet transaction intelligence.", orderRehearsal.payload.wallet_transaction_intelligence);
  assert(["missing-wallet", "not-configured", "ready", "empty", "blocked"].includes(orderRehearsal.payload.wallet_transaction_intelligence?.status), "Wallet transaction intelligence should return a known status.", orderRehearsal.payload.wallet_transaction_intelligence);
  assert(orderRehearsal.payload.wallet_transaction_intelligence?.live_execution_permission === "blocked", "Wallet transaction intelligence should not unlock live execution.", orderRehearsal.payload.wallet_transaction_intelligence);
  assert(orderRehearsal.payload.wallet_transaction_intelligence?.wallet_mutation_permission === "blocked", "Wallet transaction intelligence should keep wallet mutation blocked.", orderRehearsal.payload.wallet_transaction_intelligence);
  assert(orderRehearsal.payload.wallet_transaction_intelligence?.raw_transaction_storage === "blocked", "Wallet transaction intelligence should block raw transaction storage.", orderRehearsal.payload.wallet_transaction_intelligence);
  ...
  assert(["not-configured", "ready", "skipped", "blocked"].includes(orderRehearsal.payload.live_wallet_accounting_readiness?.asset_index_status), "Wallet accounting readiness should expose aggregate Helius DAS asset-index status.", orderRehearsal.payload.live_wallet_accounting_readiness);
  assert(orderRehearsal.payload.live_wallet_accounting_readiness?.checks?.some((check) => check.id === "asset-index"), "Wallet accounting readiness should include Helius DAS asset-index evidence.", orderRehearsal.payload.live_wallet_accounting_readiness);
