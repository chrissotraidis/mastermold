...
The research handoff command talks only to `GET /api/web3-research-handoff-packet`, checks that the returned packet includes a receipt hash, research questions, the ordered unlock sequence, open operator inputs, live-capital blockers, and the `# Mastermind Web3 Research Handoff Packet` text body, then exits nonzero if any configured secret or API-key query value appears. It cannot sign, submit, custody funds, mutate wallets, unlock live capital, or turn the research packet into trading authority.
