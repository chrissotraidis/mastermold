...
The credential action console now starts with an `Action checklist` that condenses provider evidence, public wallet scope, live DEX proof, Jupiter order proof, and live preflight into visible next controls. It follows that with a `Next operator unlock` callout populated from the operator request packet, linking directly to the public wallet field and showing the safe verifier command after the wallet is saved. Its refreshed `What is left after latest action` panel now shows the top dependency-ranked live-usability blockers from `missing_for_live_usability`, plus an `Open current gate` jump and `Open blocker JSON` link, so the operator can see the exact next public/env/review rows after each safe action. It is a checklist over existing safe actions only; it cannot sign, submit, mutate wallets, store wallet secrets, or unlock live trading.
