...
20. Run `npm run doctor:web3 -- --json` against the local app to write `data/web3-credential-doctor.json`, a sanitized local receipt that compares account setup, provider health, launch checklist, and live preflight boundaries. It stores configured/missing status, blockers, and safe commands only; it does not echo Helius/Jupiter keys, create accounts, sign, submit, or mutate wallets.
