...
19. Review the Wiring focus `Production worker review` panel after `npm run supervise:web3 -- --base-url=http://localhost:4010 --rounds=1 --ticks-per-round=1 --target-net-pnl=1 --max-drawdown=250 --json` writes a sanitized paper-supervisor receipt. It checks receipt freshness, circuit state, profit target, drawdown brake, and the live-boundary process gate, plus redacted process manager, worker owner, alert route, and restart-policy target status. It cannot install a process manager, start an external worker, dispatch alerts, or authorize live capital from inside the app.
...
The trading cockpit now opens with a `Command board` before the large autonomous workspace. That board combines the current autonomous decision, paper wallet net-worth curve, equity/PnL/exposure/drawdown metrics, next usable gate, supervised-live lane count, missing operator inputs, signal score, and route proof into one first-screen review surface. It links to `/settings/integrations` for secure setup, labels operator inputs as non-secret planning items, and still keeps live execution, transaction submission, wallet mutation, private-key storage, seed-phrase storage, and secret echo blocked. The deeper workspace still includes the autonomy mode ladder that separates copilot, paper autonomy, dry-run order rehearsal, supervised live review, and autonomous live authority for the longer telemetry review.
