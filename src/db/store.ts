...
export type PositionPolicyRow = {
  symbol: string;
  intent: "hold" | "accumulate" | "trim" | "exit" | "watch";
...
export type PortfolioBrainSnapshotRow = {
  id: string;
  source: "monarch_mcp";
  synced_at: string;
  as_of: string;
...
export interface PersistAdapter {
  readonly backend: "sqlite" | "json-file" | "memory";
  ...
  upsertPositionPolicy(policy: PositionPolicyRow): void;
  deletePositionPolicy(symbol: string): void;
  ...
  dailyReports(limit?: number): DailyReportRow[];
  upsertDailyReport(row: DailyReportRow): void;
  ...
  portfolioBrainSnapshots(limit?: number): PortfolioBrainSnapshotRow[];
  upsertPortfolioBrainSnapshot(row: PortfolioBrainSnapshotRow): void;
}
...
class SqliteAdapter implements PersistAdapter {
  ...
  constructor(db: SqliteDb) {
    ...
    this.db.run(`
      ...
      CREATE INDEX IF NOT EXISTS idx_daily_reports_run_date ON daily_reports(run_date);
      ...
      CREATE TABLE IF NOT EXISTS portfolio_brain_snapshots (
        id TEXT PRIMARY KEY,
      ...
      CREATE INDEX IF NOT EXISTS idx_portfolio_brain_synced_at ON portfolio_brain_snapshots(synced_at);
      CREATE INDEX IF NOT EXISTS idx_portfolio_brain_source ON portfolio_brain_snapshots(source);
    `);
  }
  ...
  positionPolicies(): PositionPolicyRow[] {
    return this.db
      .query("SELECT data FROM position_policies ORDER BY symbol ASC")
      .all()
      .map((row) => JSON.parse((row as { data: string }).data) as PositionPolicyRow);
  }

  upsertPositionPolicy(policy: PositionPolicyRow): void {
    this.db
      .query("INSERT OR REPLACE INTO position_policies (symbol, updated_at, data) VALUES (?, ?, ?)")
      .run(policy.symbol, policy.updated_at, JSON.stringify(policy));
  }

  deletePositionPolicy(symbol: string): void {
    this.db.query("DELETE FROM position_policies WHERE symbol = ?").run(symbol);
  }
  ...
  dailyReports(limit = 25): DailyReportRow[] {
    return this.db
      .query("SELECT id, run_date, created_at, data FROM daily_reports ORDER BY created_at DESC LIMIT ?")
      .all(limit)
      .map((row) => {
        const r = row as { id: string; run_date: string; created_at: string; data: string };
        return {
          id: r.id,
          run_date: r.run_date,
          created_at: r.created_at,
  ...
  upsertDailyReport(row: DailyReportRow): void {
    this.db
      .query("INSERT OR REPLACE INTO daily_reports (id, run_date, created_at, data) VALUES (?, ?, ?, ?)")
      .run(row.id, row.run_date, row.created_at, JSON.stringify(row.data ?? {}));
  }
  ...
  portfolioBrainSnapshots(limit = 25): PortfolioBrainSnapshotRow[] {
    return this.db
      .query("SELECT id, source, synced_at, as_of, data FROM portfolio_brain_snapshots ORDER BY synced_at DESC LIMIT ?")
      .all(limit)
      .map((row) => {
        const r = row as { id: string; source: "monarch_mcp"; synced_at: string; as_of: string; data: string };
        return {
          id: r.id,
          source: r.source,
  ...
  upsertPortfolioBrainSnapshot(row: PortfolioBrainSnapshotRow): void {
    this.db
      .query("INSERT OR REPLACE INTO portfolio_brain_snapshots (id, source, synced_at, as_of, data) VALUES (?, ?, ?, ?, ?)")
      .run(row.id, row.source, row.synced_at, row.as_of, JSON.stringify(row.data ?? {}));
  }
...
class MemoryAdapter implements PersistAdapter {
  ...
  private dailyReportRows: DailyReportRow[] = [];
  private brainRunRows: BrainRun[] = [];
  private marketMemoryRows: MarketMemoryFact[] = [];
  private portfolioBrainSnapshotRows: PortfolioBrainSnapshotRow[] = [];
  ...
  positionPolicies() {
    return [...this.positionPolicyRows].sort((a, b) => a.symbol.localeCompare(b.symbol));
  }
  upsertPositionPolicy(policy: PositionPolicyRow) {
    this.positionPolicyRows = this.positionPolicyRows.filter((item) => item.symbol !== policy.symbol);
    this.positionPolicyRows.push(policy);
  }
  deletePositionPolicy(symbol: string) {
    this.positionPolicyRows = this.positionPolicyRows.filter((item) => item.symbol !== symbol);
  }
  ...
  dailyReports(limit = 25) {
    return [...this.dailyReportRows]
      .sort((a, b) => Date.parse(b.created_at) - Date.parse(a.created_at))
      .slice(0, limit);
  }
  upsertDailyReport(row: DailyReportRow) {
    this.dailyReportRows = this.dailyReportRows.filter((item) => item.id !== row.id);
    this.dailyReportRows.push(row);
  }
  brainRuns(limit = 25) {
  ...
  portfolioBrainSnapshots(limit = 25) {
    return [...this.portfolioBrainSnapshotRows]
      .sort((a, b) => Date.parse(b.synced_at) - Date.parse(a.synced_at))
      .slice(0, limit);
  }
  upsertPortfolioBrainSnapshot(row: PortfolioBrainSnapshotRow) {
    this.portfolioBrainSnapshotRows = this.portfolioBrainSnapshotRows.filter((item) => item.id !== row.id);
    this.portfolioBrainSnapshotRows.push(row);
  }
}
...
class JsonFileAdapter implements PersistAdapter {
  // Durable JSON-file persistence for Node runtimes without bun:sqlite —
  // report it honestly so health checks don't imply data is volatile.
  ...
  positionPolicies() {
    return [...this.read().position_policies].sort((a, b) => a.symbol.localeCompare(b.symbol));
  }
  upsertPositionPolicy(policy: PositionPolicyRow) {
    const snapshot = this.read();
    snapshot.position_policies = snapshot.position_policies.filter((item) => item.symbol !== policy.symbol);
    snapshot.position_policies.push(policy);
    this.write(snapshot);
  }
  deletePositionPolicy(symbol: string) {
    const snapshot = this.read();
    snapshot.position_policies = snapshot.position_policies.filter((item) => item.symbol !== symbol);
    this.write(snapshot);
  }
  getAlertState(id: string) {
  ...
  productEvents(limit = 500) {
    return [...this.read().product_events]
      .sort((a, b) => Date.parse(b.created_at) - Date.parse(a.created_at))
      .slice(0, limit);
  }
  dailyReports(limit = 25) {
    return [...this.read().daily_reports]
      .sort((a, b) => Date.parse(b.created_at) - Date.parse(a.created_at))
      .slice(0, limit);
  }
  upsertDailyReport(row: DailyReportRow) {
    const snapshot = this.read();
    snapshot.daily_reports = snapshot.daily_reports.filter((item) => item.id !== row.id);
    snapshot.daily_reports.push(row);
    this.write(snapshot);
  }
  brainRuns(limit = 25) {
    return [...this.read().brain_runs]
      .sort((a, b) => Date.parse(b.completed_at) - Date.parse(a.completed_at))
      .slice(0, limit);
  }
  upsertBrainRun(run: BrainRun) {
  ...
  marketMemoryFacts(limit = 100) {
    return [...this.read().market_memory]
      .sort((a, b) => Date.parse(b.updated_at) - Date.parse(a.updated_at))
      .slice(0, limit);
  }
  upsertMarketMemoryFact(fact: MarketMemoryFact) {
  ...
  portfolioBrainSnapshots(limit = 25) {
    return [...this.read().portfolio_brain_snapshots]
      .sort((a, b) => Date.parse(b.synced_at) - Date.parse(a.synced_at))
      .slice(0, limit);
  }
  upsertPortfolioBrainSnapshot(row: PortfolioBrainSnapshotRow) {
    const snapshot = this.read();
    snapshot.portfolio_brain_snapshots = snapshot.portfolio_brain_snapshots.filter((item) => item.id !== row.id);
    snapshot.portfolio_brain_snapshots.push(row);
    this.write(snapshot);
  }

  // Parsing the multi-megabyte snapshot on every store call dominated page
  // render time; cache the parsed snapshot keyed by file mtime. Reader methods
  // copy the arrays they return so callers can sort/filter without touching
  // the cached object.
  private cache: { snapshot: StoreSnapshot; mtimeMs: number } | null = null;
  ...
  private read(): StoreSnapshot {
    try {
      ...
      const snapshot: StoreSnapshot = {
        ...
        daily_reports: Array.isArray(parsed.daily_reports) ? parsed.daily_reports : [],
        ...
        portfolio_brain_snapshots: Array.isArray(parsed.portfolio_brain_snapshots) ? parsed.portfolio_brain_snapshots : [],
      };
      this.cache = { snapshot, mtimeMs };
      return snapshot;
    } catch {
      // CRASH SAFETY (2026-07-02 store-loss lesson): never surface an empty
      // snapshot for an existing store — the next write would clobber real
      // holdings/journal data. Last good cache wins; a real but unreadable
      // file throws instead of defaulting.
      if (this.cache) return this.cache.snapshot;
      if (existsSync(this.path)) {
        throw new Error(`advisory store at ${this.path} exists but is unreadable — refusing to default over real data`);
      }
  ...
  private write(snapshot: StoreSnapshot) {
    // Atomic: temp file + rename, so a crash mid-write can't tear the store.
    const tmp = `${this.path}.tmp-${process.pid}`;
    writeFileSync(tmp, JSON.stringify(snapshot, null, 2));
    renameSync(tmp, this.path);
    try {
      this.cache = { snapshot, mtimeMs: statSync(this.path).mtimeMs };
    } catch {
      this.cache = null;
    }
  }
