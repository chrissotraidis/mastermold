...
export function getBrainContextForInference(asOf: AsOfFilter | null = null) {
  ...
  return {
    ...
    memory_facts: state.facts.slice(0, 10).map((fact) => ({
      symbol: fact.symbol,
...
function buildBaseFacts(now: string): MarketMemoryFact[] {
  ...
  if (topHolding) {
    facts.push({
      ...
      confidence: 0.82,
      source_count: portfolio.manual_holdings.length > 0 ? portfolio.manual_holdings.length : demoDatabase.holdings.length,
      evidence_urls: [],
      created_at: now,
      updated_at: now,
