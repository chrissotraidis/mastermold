...
export async function importPortfolioFromProvider(
  ...
  if (!["coinbase", "robinhood", "onchain_wallet"].includes(service)) {
    throw new Error("Choose Coinbase, Robinhood, or an on-chain wallet to import.");
  }
...
function snapTradePositionToRow(
  ...
  return {
    ...
    service: "robinhood",
    account_id: accountId,
    account_label: stringAt(account, "institution_name") || "Robinhood import",
    symbol,
    asset_name:
      firstText(
...
function importMessage(service: PortfolioImportService, importedCount: number, skippedCount: number) {
  ...
  const importedText =
    importedCount > 0
      ? `${provider} imported ${importedCount} holding${importedCount === 1 ? "" : "s"} from a snapshot`
...
function providerLabel(service: PortfolioImportService) {
  ...
  if (service === "robinhood") return "SnapTrade";
  return "Zerion";
}
