...
const runMetaSchema = z.object({
  ...
  // Optional free-news stage output (engine >= 2026-07-02); older bundles omit it.
  news: z
    .array(
      z.object({
        symbol: z.string(),
...
export function ingestEngineRun(bundle: EngineBundle): boolean {
  const ingested = store().markRunIngested(
    bundle.run.run_date,
    bundle.run.knowledge_time,
    bundle.run,
  );
  if (ingested) recordRunMemoryFacts(bundle);
  return ingested;
}

/**
 * Research memory: every ingested run leaves durable per-symbol facts in
 * market_memory, so the brain accumulates a queryable history across days
 * (what moved, what the read concluded) instead of only holding the latest
 * bundle. Runs once per run_date because it is gated on fresh ingestion.
 */
function recordRunMemoryFacts(bundle: EngineBundle): void {
  const now = new Date().toISOString();
  const runDate = bundle.run.run_date;
  const base = {
    event_time: bundle.run.event_time,
    knowledge_time: bundle.run.knowledge_time,
    created_at: now,
    updated_at: now,
    source_count: 1,
  ...
  for (const alert of bundle.alerts) {
    const symbol = symbolFromAssetId(alert.asset_id);
    store().upsertMarketMemoryFact({
      ...base,
      id: `fact_${runDate}_${symbol ?? alert.id}_signal`,
      symbol,
      topic: "market",
      summary: `${alert.message} ${alert.rationale}`.trim(),
      confidence: Math.min(10, Math.round(Math.abs(alert.z_score) * 3)),
    });
  }

  for (const card of bundle.briefing_cards) {
    if (card.status !== "actionable") continue;
    const symbol = symbolFromAssetId(card.asset_ids[0] ?? null);
    store().upsertMarketMemoryFact({
      ...base,
      id: `fact_${runDate}_${symbol ?? card.id}_read`,
      symbol,
      topic: "market",
      summary: `${card.headline} ${card.why_now}`.trim(),
      confidence: card.conviction,
    });
  }

  (bundle.run.news ?? []).forEach((item, index) => {
    store().upsertMarketMemoryFact({
      ...base,
      evidence_urls: item.url ? [item.url] : [],
      id: `fact_${runDate}_${item.symbol}_news_${index}`,
      symbol: item.symbol.toUpperCase(),
      topic: "news",
      summary: item.publisher ? `${item.headline} (${item.publisher})` : item.headline,
      confidence: 5,
    });
  });
}

function symbolFromAssetId(assetId: string | null): string | null {
  if (!assetId) return null;
  const raw = assetId.startsWith("asset_") ? assetId.slice("asset_".length) : assetId;
  return /^[A-Za-z0-9]{1,12}$/.test(raw) ? raw.toUpperCase() : null;
}
